...
public class RigidBodyPlatform : MonoBehaviour
{
    ...
    private Rigidbody rb; // reference to platform rigidbody
    ...
    private void FixedUpdate()
    {
        if (!waiting)
        {
            if (forth)
            {
                ...
                rb.MovePosition(Vector3.Lerp(start, end, Mathf.Pow(percentageComplete, power)));
            ...
            else if (back)
            {
                ...
                float currentTime = Time.time - startTime;
                float percentageComplete = currentTime / timeToLerp;
                rb.MovePosition(Vector3.Lerp(end, start, Mathf.Pow(percentageComplete, power)));
